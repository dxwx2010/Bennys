...
local function saveVehicle()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        TriggerEvent("pnp-jobs:bennys:order", veh, changedStuff)
        return
    end
    local vehId = exports["pnp-vehicles"]:GetVehicleIdentifier(veh)
    TriggerServerEvent("updateVehicle", vehicleMods, vehId)
end
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    if bennys.access and (bennys.access == "police" or bennys.access == "ems") and purchaseType == "repair" then
        return RPC.execute("pnp-bennys:jobRepair")
    end
    ...
    if (bennys.access == "police" or bennys.access == "ems") then
        local free = exports['pnp-config']:GetMiscConfig('bennys.pd.free')
        pd = free
        return pd
    end

    TriggerServerEvent("pnp-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if (missingEngineHealth > 50) then
            local finished = exports["pnp-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Repairing engine...", true)
        ...
        if missingBodyHealth > 50 then
            local finished = exports["pnp-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true)
...
function PreviewMod(categoryID, modID)
    ...
    if categoryID == 39 or categoryID == 4 then
        if GetResourceState("pnp-vehiclesync-state") == "started" then
            exports['pnp-vehiclesync-state']:SetEngineSound(plyVeh, true)
        elseif GetResourceState("pnp-vehiclesync") == "started" then
            exports['pnp-vehiclesync']:SetEngineSound(plyVeh, true)
        end
    end
end
...
function ExitBennys()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        if GetResourceState("pnp-vehiclesync-state") == "started" then
            exports['pnp-vehiclesync-state']:SetEngineSound(plyVeh)
        elseif GetResourceState("pnp-vehiclesync") == "started" then
            exports['pnp-vehiclesync']:SetEngineSound(plyVeh)
        end
    ...
    TriggerServerEvent("pnp-bennys:removeFromInUse", currentBennys)
    if devmode then
      TriggerEvent("vehicle:leftBennys", nil, nil)
    elseif not isAtPreviewBennys then
      exports["pnp-ui"]:showInteraction("Bennys")
...
local function finishEnterLocation()
    ...
    isPlyInBennys = true
    TriggerServerEvent("pnp-bennys:addToInUse", currentBennys)
    disableControls()
    TriggerEvent("inmenu", isPlyInBennys)
end
...
function enterLocation(pBennys, needsRepair)
    ...
    if isAtEmployedBennys then
        ...
        VehicleClass = exports["pnp-vehicles"]:GetVehicleRatingClass(plyVeh)
    end
    ...
    isVinScratched = exports['pnp-vehicles']:IsVinScratched(plyVeh)
...
--#[Event Handlers]#--
RegisterNetEvent("pnp-bennys:purchaseSuccessful")
AddEventHandler("pnp-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("pnp-bennys:purchaseFailed")
AddEventHandler("pnp-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    local isBennysInUse = RPC.execute("pnp-bennys:checkIfUsed", currentBennys)
    ...
    local isTestDrive = false--RPC.execute("showroom:isTestDriveVehicle", NetworkGetNetworkIdFromEntity(plyVeh))
    ...
    if bennysAccess then
        ...
        jobAccess = exports["pnp-business"]:IsEmployedAt(bennysAccess)

        if (isAtPreviewBennys and previewPermission[currentBennys] and isTestDrive) then
            jobAccess = exports["pnp-business"]:HasPermission(bennysAccess, previewPermission[currentBennys])
        end
...
Citizen.CreateThread(function()
    Wait(5000)
    local catalog = RPC.execute('pnp-jobs:bennys:getPartsCatalog')

    local modifiers = RPC.execute('pnp-jobs:bennys:getPriceModifiers')

    local taxLevel = RPC.execute("pnp-financials:getTax", "Services")
...
RegisterNetEvent("pnp-admin:currentDevmode")
AddEventHandler("pnp-admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)
...
RegisterNetEvent('pnp-admin:bennys', function()
    devmode = true
